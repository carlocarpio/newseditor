...
const NewsEditor = (props) => {
  ...
  const handleContent = (details) => {
    ...
    db.collection("NewsPost").doc(activeId).update({
      title: details.title,
  ...
  const getItems = async (url) => {
    await firebase.firestore().collection('NewsPost').where("url", "==", url).get().then((querySnapshot) => {
      const data = querySnapshot.docs[0].data() && querySnapshot.docs[0].data()
      querySnapshot.docs[0].id !== undefined && setactiveId(querySnapshot.docs[0].id)
      setDetails({
        title: data.title,
  ...
  React.useEffect(() => {
    getItems(props.match.params.url)
  }, [])
  ...
  return (
    ...
    <div className="container mx-auto py-10">
      <div className="flex justify-between">
        <div className="py-10"><h2 className="text-4xl">Edit News</h2></div>
        <div className="py-10">
        ...
        <div>
          ...
          <div className="w-full px-3 mb-6">
            <label className="block uppercase tracking-wide text-xs font-bold mb-2 text-gray-400" for="grid-last-name">
                URL
            </label>
            <input disabled value={details.url} onChange={(e) => setDetails({ ...details, url: e.target.value })} className="appearance-none block w-full bg-gray-400 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-last-name" type="text" />
            </div>
